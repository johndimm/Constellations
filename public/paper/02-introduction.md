...
## Contributions (Draft)
- **A bipartite exploration interface** that enforces Atomic↔Composite alternation to keep interaction simple across domains.
- **An LLM-assisted, on-demand graph construction pipeline** that builds local neighborhoods without a precomputed knowledge graph.
- **Evidence-backed edges** that make “why is this connected?” a first-class interaction.
- **An evaluation plan** targeting low-friction exploration outcomes (recall, discovery, orientation, trust), rather than accuracy alone.
