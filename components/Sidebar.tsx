...
import { X, ExternalLink, Flag, Target, ChevronUp, ChevronDown } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ selectedNode, onClose, onSetStart, onSetEnd }) => {
  ...
  const desktopClasses = `fixed top-4 right-4 z-20 w-[24rem] transition-transform duration-300 ease-in-out ${
    isCollapsed ? 'translate-x-[calc(100%+2rem)]' : 'translate-x-0'
  }`;

  // Mobile Bottom Drawer Styling
  const mobileClasses = `fixed bottom-0 left-0 right-0 z-30 transition-transform duration-300 ease-in-out ${
    isCollapsed ? 'translate-y-[calc(100%-3.5rem)]' : 'translate-y-0'
  }`;

  return (
    <div className={isMobile ? mobileClasses : desktopClasses}>
      <div className={`bg-slate-900/95 backdrop-blur-xl border-slate-700 shadow-2xl relative pointer-events-auto flex flex-col ${
        isMobile 
